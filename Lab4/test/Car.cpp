  		...
  		switch(event.type) {
			case SDL_JOYAXISMOTION:
				if (event.jaxis.axis == 0) {
					//if ((old_value + 10000 > event.jaxis.value) || (old_value - 10000< event.jaxis.value))
					steering = event.jaxis.value;
					//old_value = event.jaxis.value;
				}
				else if(event.jaxis.axis == 5) {
					motor = event.jaxis.value;
				}
				break;
			case SDL_QUIT:
				std::cout << "quitting\n";
				...
				break;
			default:
				//std::cout << "I dont care about this event hahahahaha\n";
				break;
		}
	}
	if (something_new){
